...
Item_category_1::Item_category_1(QJsonArray* arr) {
    ...
    //qDebug()<<"222";
    rectHover = QRect(0, 0,  600, 80);
    rectRate = QRect(480, -40,  40, 40);

    update();
}

int Item_category_1::getHeight()
{
    return arrCount * 40;
}

QRectF Item_category_1::boundingRect() const
{
    qreal penWidth = 1;
    return QRectF(penWidth / 2 - 10, penWidth / 2 - 10 - 40,
                  600 + penWidth + 20, 40 * 1 + penWidth + 10 + 40 * 2);
}

void Item_category_1::paint(QPainter *painter,
                              ...
                              QWidget*)
{
    ...
    if(workFlag)
        painter->fillRect(rectHover, "red");
    else
        if(hoverFlag)
            painter->fillRect(rectHover, "lightgray");
    ...
    for(int i = 0; i < arrCount; i++){
        ...
        QString addRate = QString::number(jArr->at(i).toObject().value("AddRate").toInt());
...
void Item_category_1::mousePressEvent(QGraphicsSceneMouseEvent * e)
{
    int x = e->pos().x();
    int y = e->pos().y();
    ...
    // }
        ...
        if(rectHover.contains(x, y)){
            workFlag = true;
            QJsonObject o;
            o.insert("Red", jArr->at(0));
            o.insert("Blue", jArr->at(1));
            emit sigSendToControlPanel(o);
        }
...
void Item_category_1::hoverMoveEvent(QGraphicsSceneHoverEvent * e)
{
    int x = e->pos().x();
    int y = e->pos().y();
    for(int i = 0; i < 2; i++){
