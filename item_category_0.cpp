...
Item_category_0::Item_category_0(QJsonArray* arr) {
...
int Item_category_0::getHeight()
{
    return arrCount * 40;
}

QRectF Item_category_0::boundingRect() const
{
    qreal penWidth = 1;
...
void Item_category_0::paint(QPainter *painter,
...
void Item_category_0::mousePressEvent(QGraphicsSceneMouseEvent * e)
{
    int x = e->pos().x();
    int y = e->pos().y();
...
void Item_category_0::hoverMoveEvent(QGraphicsSceneHoverEvent * e)
{
    int x = e->pos().x();
    int y = e->pos().y();
    for(int i = 0; i < lRectHover.count(); i++){
